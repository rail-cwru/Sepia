...
public void oneMoves(int player1, int player2) throws FileNotFoundException, JSONException {
		...
		assertTrue("One should have succeeded, but did not",model.getHistory(player1).getActionsExecuted(player1).getActions(lastturn).size()==1);
	...
	public void twoMoves(int player1, int player2) throws FileNotFoundException, JSONException {
		...
		model.executeStep();
		int lastturn = s.getTurnNumber()-1;
		assertTrue("They should have failed, but did not",model.getHistory(player1).getActionsExecuted(player1).getActions(lastturn).size()==0);
		assertTrue("They should have failed, but did not",model.getHistory(player2).getActionsExecuted(player2).getActions(lastturn).size()==0);
	}
	public void threeMoves(int player1, int player2, int player3) throws FileNotFoundException, JSONException {
		...
		assertTrue("They should have failed, but did not",model.getHistory(player1).getActionsExecuted(player1).getActions(lastturn).size()==0);
		assertTrue("They should have failed, but did not",model.getHistory(player2).getActionsExecuted(player2).getActions(lastturn).size()==0);
		assertTrue("They should have failed, but did not",model.getHistory(player3).getActionsExecuted(player3).getActions(lastturn).size()==0);
	}
	/**
	...
	public void shootAndKillWhileMovingAway() throws FileNotFoundException, JSONException {
		...
		model.executeStep();
		int lastturn = s.getTurnNumber()-1;
		assertTrue("They should have succeeded, but did not",model.getHistory(player1).getActionsExecuted(player1).getActions(lastturn).size()==1);
		assertTrue("They should have succeeded, but did not",model.getHistory(player2).getActionsExecuted(player2).getActions(lastturn).size()==1);
	...
	public void shootAndKillWhileMovingAwayButStillBlockOtherMove() throws FileNotFoundException, JSONException {
		...
		int lastturn = s.getTurnNumber()-1;
		assertTrue("They should have succeeded, but did not",model.getHistory(player1).getActionsExecuted(player1).getActions(lastturn).size()==1);
		assertTrue("Move should have failed, but did not. it is now at "+u2.getxPosition()+","+u2.getyPosition(),model.getHistory(player2).getActionsExecuted(player2).getActions(lastturn).size()==0);
		assertTrue("Move should have failed, but did not. it is now at "+u3.getxPosition()+","+u3.getyPosition(),model.getHistory(player3).getActionsExecuted(player3).getActions(lastturn).size()==0);
		assertNull("The target should be dead but isn't",model.getState(player1).getUnit(u2.ID));
	}
	/**
...
//		System.out.println(Arrays.toString(model.getHistory(Agent.OBSERVER_ID).getActionResults(player2).getActionResults(lastturn).toArray(new ActionResult[0])));
		assertTrue("One should have failed, but did not",model.getHistory(player1).getActionsExecuted(player1).getActions(lastturn).size()==0);
		assertTrue("They should have succeeded, but did not",model.getHistory(player2).getActionsExecuted(player2).getActions(lastturn).size()==1);
	}
	/**
...
//		System.out.println(Arrays.toString(model.getHistory(Agent.OBSERVER_ID).getActionResults(player2).getActionResults(lastturn).toArray(new ActionResult[0])));
		assertTrue("One should have failed, but did not",model.getHistory(player1).getActionsExecuted(player1).getActions(lastturn).size()==0);
		assertTrue("They should have succeeded, but did not",model.getHistory(player3).getActionsExecuted(player3).getActions(lastturn).size()==1);
...
//		System.out.println(r2.getxPosition()+","+r2.getyPosition());
		assertTrue("Move should have failed, but did not",model.getHistory(player1).getActionsExecuted(player1).getActions(lastturn).size()==0);
		assertTrue("Gather should have succeeded, but did not",model.getHistory(player3).getActionsExecuted(player3).getActions(lastturn).size()==1);
...
//		System.out.println(r2.getxPosition()+","+r2.getyPosition());
		...
		assertTrue("On "+amount+" Gather1 should have "+(shouldhavesucceeded?"succeeded":"failed")+", but did not",model.getHistory(player1).getActionsExecuted(player1).getActions(lastturn).size()==(shouldhavesucceeded?1:0));
		assertTrue("On "+amount+" Gather2 should have "+(shouldhavesucceeded?"succeeded":"failed")+", but did not",model.getHistory(player3).getActionsExecuted(player3).getActions(lastturn).size()==(shouldhavesucceeded?1:0));
...
//		System.out.println(r2.getxPosition()+","+r2.getyPosition());
		...
		boolean nodeshouldremain = amount != Amount.THESAMEASATTEMPT;
		assertTrue("On "+amount+" Gather1 should have "+(shouldhavesucceeded?"succeeded":"failed")+", but did not",model.getHistory(player1).getActionsExecuted(player1).getActions(lastturn).size()==(shouldhavesucceeded?1:0));
		assertTrue("On "+amount+" Gather2 should have "+(shouldhavesucceeded?"succeeded":"failed")+", but did not",model.getHistory(player2).getActionsExecuted(player2).getActions(lastturn).size()==(shouldhavesucceeded?1:0));
		assertTrue("On "+amount+" Gather3 should have "+(shouldhavesucceeded?"succeeded":"failed")+", but did not",model.getHistory(player3).getActionsExecuted(player3).getActions(lastturn).size()==(shouldhavesucceeded?1:0));
		assertTrue("On "+amount+" The node should be "+(nodeshouldremain?"still there":"exhausted")+" but isn't",(nodeshouldremain?model.getState(Agent.OBSERVER_ID).getResourceNode(r2.ID)!=null:model.getState(Agent.OBSERVER_ID).getResourceNode(r2.ID)==null));
	}
	/**
	...
	public void NProduceMRoomLeft(int n, int m, Amount amount) throws FileNotFoundException, JSONException
	{
		...
		model.executeStep();
		int lastturn = s.getTurnNumber()-1;
		int amountactuallysucceeded=model.getHistory(player1).getActionsExecuted(player1).getActions(lastturn).size();
...
//		new VisualAgent(player1,new String[]{"true","true"}).initialStep(model.getState(player1), model.getHistory(player1));
		int totalactuallydone = model.getHistory(player1).getActionsExecuted(player1).getActions(lastturn).size();
